...
double UI::tileAnimationFrameDuration = 0.15;
...
void UI::RenderControlPanel(Engine& engine, const Grid& grid) {
    ...
    std::string controlPanelTitle = std::string(TR("control_panel")) + "##MainWindow";
    ImGui::Begin(controlPanelTitle.c_str(), nullptr);

    Language currentLanguage = Localization::GetLanguage();
    Language nextLanguage = (currentLanguage == Language::English) ? Language::Slovak : Language::English;
    std::string languageButtonLabel =
        std::string(TR("language")) + ": " + Localization::GetLanguageDisplayName(currentLanguage) + "##LanguageToggle";
    if (ImGui::Button(languageButtonLabel.c_str(), ImVec2(-1, 0))) {
    ...
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip("%s", TR("import_asset_tooltip"));
    }
    if (ImGui::Button(TR("tutorial.button"), ImVec2(-1, 35))) {
        tutorialState.isOpen = true;
    }
...
void UI::RenderTutorialWindow() {
    if (!tutorialState.isOpen) {
        return;
    }

    static constexpr std::array<const char*, 13> titleKeys = {
    ...
    static constexpr std::array<const char*, 13> descriptionKeys = {
        "tutorial.step1.desc",
        "tutorial.step2.desc",
        "tutorial.step3.desc",
        "tutorial.step4.desc",
        ...
        "tutorial.step8.desc",
        "tutorial.step9.desc",
        "tutorial.step10.desc",
        "tutorial.step11.desc",
        "tutorial.step12.desc",
        "tutorial.step13.desc"
    };

    ImGuiIO& io = ImGui::GetIO();
    const float panelWidth = io.DisplaySize.x * controlPanelWidthRatio;
    const float scenesBarHeight = io.DisplaySize.y / 18.0f;
    const float consoleHeight = io.DisplaySize.y * assetConsoleHeightRatio;

    const float availableWidth = std::max(260.0f, io.DisplaySize.x - panelWidth - 20.0f);
    const float availableHeight = std::max(220.0f, io.DisplaySize.y - scenesBarHeight - consoleHeight - 20.0f);
    const float windowWidth = std::min(450.0f, availableWidth);
    const float windowHeight = std::min(340.0f, availableHeight);

    ImGui::SetNextWindowPos(ImVec2(panelWidth + 10.0f, scenesBarHeight + 10.0f), ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowSize(ImVec2(windowWidth, windowHeight), ImGuiCond_FirstUseEver);

    std::string tutorialTitle = std::string(TR("tutorial.window_title")) + "##TutorialWindow";
    if (!ImGui::Begin(tutorialTitle.c_str(), &tutorialState.isOpen, ImGuiWindowFlags_NoCollapse)) {
        ImGui::End();
        return;
    }

    const int totalSteps = static_cast<int>(titleKeys.size());
    tutorialState.currentStep = std::clamp(tutorialState.currentStep, 0, totalSteps - 1);

    const int displayStep = tutorialState.currentStep + 1;
    ImGui::Text(TR("tutorial.progress"), displayStep, totalSteps);
    ImGui::Separator();

    ImGui::Text("%s", TR(titleKeys[tutorialState.currentStep]));
    ImGui::Spacing();
    ImGui::TextWrapped("%s", TR(descriptionKeys[tutorialState.currentStep]));

    ImGui::Dummy(ImVec2(0.0f, 10.0f));
    ImGui::Separator();

    const bool canGoBack = tutorialState.currentStep > 0;
    const bool canGoNext = tutorialState.currentStep < totalSteps - 1;

    if (!canGoBack) {
        ImGui::BeginDisabled();
    }
    if (ImGui::Button(TR("tutorial.back"), ImVec2(85, 0)) && canGoBack) {
    ...
    if (ImGui::Button(TR("tutorial.next"), ImVec2(85, 0)) && canGoNext) {
        tutorialState.currentStep++;
    }
    if (!canGoNext) {
        ImGui::EndDisabled();
    ...
    ImGui::SameLine();
    if (ImGui::Button(TR("tutorial.restart"), ImVec2(85, 0))) {
        tutorialState.currentStep = 0;
    }

    ImGui::SameLine();
    if (ImGui::Button(TR("tutorial.close"), ImVec2(85, 0))) {
        tutorialState.isOpen = false;
    }
...
void UI::RenderDebugConsole() {
    ...
    std::string debugConsoleTitle = std::string(TR("debug_console")) + "##DebugWindow";
    ImGui::Begin(debugConsoleTitle.c_str(), nullptr);
    if (ImGui::Button(TR("clear_console"))) {
        ClearDebugMessages();
    }
    ImGui::SameLine();
    ImGui::Text(TR("messages_count"), DebugMessages.size());
...
void UI::RenderAssetConsole(Engine& engine) {
    ...
    std::string assetConsoleTitle = std::string(TR("asset_console")) + "##AssetConsoleWindow";
    ImGui::Begin(assetConsoleTitle.c_str(), nullptr,
                ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize);

    AssetManager& assetManager = engine.GetAssetManager();

    if (ImGui::Button(TR("import_new_asset"), ImVec2(150, 30))) {
        engine.GetAssetImporter().SetOpen(true);
    }
    ImGui::SameLine();
    ImGui::Separator();
...
void UI::RenderAssetFilters(AssetManager& assetManager, const std::vector<std::string>& categories) {
    ...
    if (ImGui::BeginCombo(TR("category"), selectedCategory.c_str())) {
        if (ImGui::Selectable(TR("all"), selectedCategory == "All")) {
    ...
    const char* typeNames[] = {
        TR("asset_type.all"),
        TR("asset_type.texture"),
        TR("asset_type.animated_sprite"),
        TR("asset_type.static_sprite"),
        TR("asset_type.tileset"),
        TR("asset_type.player"),
        TR("asset_type.enemy"),
        TR("asset_type.other")
    };
    ...
    if (ImGui::Combo(TR("type"), &currentTypeIndex, typeNames, IM_ARRAYSIZE(typeNames))) {
    ...
    ImGui::SliderInt(TR("thumb_size"), &thumbnailSize, 85, 120);
    ImGui::SameLine();

    // Asset count
    ImGui::Text(TR("assets_count"), assetManager.GetAssetCount());
    ImGui::SameLine();

    ImGui::SetNextItemWidth(100);
    ImGui::SliderFloat(TR("console_size"), &UI::assetConsoleHeightRatio, 0.15f, 0.60f, "%.2f");
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", TR("console_size_tooltip"));
}

void UI::RenderCategoryList(const std::vector<std::string>& categories, std::vector<Asset*>& allAssets) {
    ImGui::Text("%s", TR("categories"));
    ImGui::Separator();

    // "All" category
    const int allCount = static_cast<int>(allAssets.size());
    std::string allLabel = std::string(TR("all")) + " (" + std::to_string(allCount) + ")";
...
void UI::RenderAssetGrid(Engine& engine, AssetManager& assetManager, const std::vector<Asset*>& allAssets) {
    ...
    if (filteredAssets.empty()) {
        ImGui::Text("%s", TR("no_assets_found"));
        return;
    }
    ...
    ImGui::Text(TR("found_assets"), filteredAssets.size());
    ImGui::Separator();
...
void UI::RenderAssetThumbnail(Asset* asset) {
    ...
    if (asset->loaded) {
        ...
        const char* notLoadedText = TR("not_loaded");
    ...
    if (hovered) {
        ImGui::BeginTooltip();
        ImGui::Text(TR("name"), asset->name.c_str());
        ImGui::Text(TR("id"), asset->id.c_str());
        ImGui::Text(TR("type_value"), GetAssetTypeName(asset->type).c_str());
        ImGui::Text(TR("category_value"), asset->category.c_str());
...
void UI::RenderAssetDetails(Engine& engine, Asset* asset, AssetManager& assetManager) {
    ImGui::Text("%s", TR("selected_asset_details"));
    ImGui::Separator();
    ...
    if (asset->loaded) {
        ...
        if (asset->SpriteSourceRect.width > 0) {
            ImGui::TextColored(ImVec4(0.2f, 1.0f, 0.2f, 1.0f), "%s", TR("selected_frame"));
        }
    }
    ...
    ImGui::Text(TR("type_value"), GetAssetTypeName(asset->type).c_str());
    ImGui::Text(TR("category_value"), asset->category.c_str());
    ImGui::Text(TR("path"), asset->path.c_str());

    if (asset->loaded) {
        ImGui::Text(TR("texture_size"), asset->texture.width, asset->texture.height);

        // Show frame info if available
        if (asset->SpriteSourceRect.width > 0 && asset->SpriteSourceRect.height > 0) {
            ImGui::Separator();
            ImGui::TextColored(ImVec4(0.2f, 1.0f, 0.2f, 1.0f), "%s", TR("selected_frame_label"));
            ImGui::Text(TR("size"),
                       asset->SpriteSourceRect.width,
                       asset->SpriteSourceRect.height);
            ...
            if (asset->SpriteSize.x > 0 && asset->SpriteSize.y > 0) {
                ...
                ImGui::Text(TR("frame_col_row"), frameX, frameY);
            }
        }
        ...
        if (asset->type == AssetType::TILESET) {
            ImGui::Separator();
            ImGui::Text(TR("tileset_tiles"), asset->subSprites.size());
            ...
            if (!asset->subSprites.empty()) {
                ...
                ImGui::Text(TR("grid_tiles"), cols, rows);
            }
        }
    } else {
        ImGui::TextColored(ImVec4(1, 0, 0, 1), "%s", TR("not_loaded_caps"));
    }

    // Use Asset button
    if (asset->type == AssetType::TILESET) {
        if (ImGui::Button(TR("select_tile"), ImVec2(120, 30))) {
        ...
        ImGui::SameLine();
        if (ImGui::Button(TR("tile_animations"), ImVec2(120, 30))) {
        ...
        ImGui::SameLine();
        if (ImGui::Button(TR("remove_asset"), ImVec2(120, 30))) {
    ...
    if (asset->type == AssetType::ANIMATED_SPRITESHEET ||
        ...
        if (asset->loaded && asset->texture.id != 0) {
            ...
            if (ImGui::Button(TR("edit_animations"), ImVec2(120, 30))) {
    ...
    if (asset->type != AssetType::TILESET) {
        ImGui::Spacing();
        ImGui::Separator();
        ImGui::Text("%s", TR("placed_asset_settings"));
        ...
        if (scene) {
            ...
            if (matchingEntities.empty()) {
                ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1), "%s", TR("no_instances_placed"));
            } else {
                ImGui::Text(TR("placed_instances"), (int)matchingEntities.size());

                ImGui::Spacing();
                if (ImGui::Button(TR("select_in_scene"), ImVec2(-1, 30))) {
                ...
                if (selectedSceneEntity &&
                    ...
                    ImGui::Spacing();
                    ImGui::Separator();
                    ImGui::TextColored(ImVec4(0.2f, 1.0f, 0.2f, 1), "%s", TR("selected_instance"));

                    if (auto* staticEntity = dynamic_cast<StaticEntity*>(selectedSceneEntity)) {
                        ImGui::Text(TR("instance_type"), TR("asset_type.static_sprite"));
                        ImGui::Text(TR("instance_position"), staticEntity->GetGridX(), staticEntity->GetGridY());
                        ImGui::Text(TR("layer"), staticEntity->GetLayer());
                    } else if (auto* playerEntity = dynamic_cast<PlayerEntity*>(selectedSceneEntity)) {
                        ImGui::Text(TR("instance_type"), TR("asset_type.player"));
                        ImGui::Text(TR("instance_position"), playerEntity->GetGridX(), playerEntity->GetGridY());
                        ImGui::Text(TR("layer"), playerEntity->GetLayer());
                    } else if (auto* enemyEntity = dynamic_cast<EnemyEntity*>(selectedSceneEntity)) {
                        ImGui::Text(TR("instance_type"), TR("asset_type.enemy"));
                        ImGui::Text(TR("instance_position"), enemyEntity->GetGridX(), enemyEntity->GetGridY());
                        ImGui::Text(TR("layer"), enemyEntity->GetLayer());
                    }

                    float currentScale = selectedSceneEntity->GetScale();
                    ImGui::Text(TR("current_scale"), currentScale);

                    ImGui::Spacing();
                    ImGui::Text("%s", TR("adjust_scale"));
                    ...
                    if (matchingEntities.size() > 1) {
                        ImGui::Text("%s", TR("apply_all_instances"));
                        if (ImGui::Button(TR("scale_all_to_match"), ImVec2(-1, 30))) {
                    ...
                    ImGui::Spacing();
                    if (ImGui::Button(TR("deselect"), ImVec2(-1, 25))) {
                        selectedSceneEntity = nullptr;
...
void UI::RenderTileSceneContext(Engine& engine, const Grid& grid) {
            ...
            if (Scene *scene = engine.GetCurrentScene()) {
                ...
                if (tileMap.HasTile(gridX, gridY)) {
                    ...
                    for (const auto& entity : entities) {
                        ...
                        } else if (const auto enemy = dynamic_cast<EnemyEntity*>(entity.get())) {
    ...
    if (ImGui::BeginPopup("TileContextMenu")) {
        ...
        if (!tileVecPtr || tileVecPtr->empty()) {
            ImGui::Text("%s", TR("no_tile_at_position"));
            ImGui::EndPopup();
            return;
        }
        ImGui::Text(TR("tile_coords"), contextTileX, contextTileY);
        ImGui::Separator();
        ...
        if (!tile && !tileVecPtr->empty()) {
            tile = &(*tileVecPtr)[0];
            ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.0f, 1.0f), TR("tile_from_other_layer"), tile->layer, currentLayer);
        }
        if (tile) {
            bool isSolid = tile->isSolid;
            if (ImGui::Checkbox(TR("solid"), &isSolid)) {
            ...
            ImGui::Separator();
            ImGui::TextColored(ImVec4(0.6f, 0.8f, 1.0f, 1.0f), "%s", TR("bulk_collision_edit"));

            if (!bulkEditActive) {
                if (ImGui::Button(TR("start_selection"), ImVec2(-1, 0))) {
                ...
                ImGui::TextDisabled("%s", TR("start_rect_selection_hint"));
            } else {
                ...
                ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "%s", TR("selection_active"));
                ImGui::Text(TR("selection_from_to"),
                           bulkEditStartX, bulkEditStartY,
                           bulkEditEndX, bulkEditEndY);
                ImGui::Text(TR("selection_grid_cells"),
                           (maxX - minX + 1), (maxY - minY + 1));
                ImGui::Text(TR("selection_tiles_empty"), tilesWithData, emptyPositions);
                ImGui::TextDisabled("%s", TR("right_click_more_tiles"));

                ImGui::Separator();

                // Enable Collision button
                if (ImGui::Button(TR("enable_collision"), ImVec2(-1, 0))) {
                    int changed = 0;
                ...
                if (ImGui::Button(TR("disable_collision"), ImVec2(-1, 0))) {
                    int changed = 0;
                ...
                if (ImGui::Button(TR("cancel_selection"), ImVec2(-1, 0))) {
                    bulkEditActive = false;
            ...
            if (ImGui::Checkbox(TR("scene_switch"), &isSceneSwitch)) {
            ...
            if (tile->isSceneSwitcher) {
                ...
                if (ImGui::Combo(TR("trigger_key"), &currentKeyIndex, keyNames, keyOptionCount)) {
                    tile->triggerKey = keyOptions[currentKeyIndex].key;
                    tileMapModified = true;
                }
                auto& scenes = engine.GetAllScenes();
                if (scenes.size() <= 1) {
                    ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "%s", TR("no_other_scenes_available"));
                }
                else {
                    ...
                    if (ImGui::Combo(TR("target_scene"), &currentSceneIndex, sceneLabels.data(), static_cast<int>(sceneLabels.size()))) {
...
void UI::RenderLayerVisibilityControls(Engine &engine) {
    ...
    for (int i = 0; i < currentLayers; i++) {
        ...
        if (ImGui::BeginPopupContextItem()) {
            ...
            ImGui::Text(TR("layer_popup_title"), i);
            ImGui::Separator();

            if (ImGui::MenuItem(isVisible ? TR("layer_hide") : TR("layer_show"), nullptr, isVisible)) {
                engine.ToggleLayerVisibility(i);
            }

            if (ImGui::MenuItem(TR("layer_set_active"))) {
                engine.SetCurrentTileLayer(i);
            }
            ...
            if (ImGui::IsItemHovered() && !canRemoveThisLayer) {
                ImGui::SetTooltip("%s", TR("layer_delete_disabled_tooltip"));
            }
        ...
        if (ImGui::IsItemHovered()) {
            ImGui::BeginTooltip();
            ImGui::Text(TR("layer_tooltip_title"), i);
            ImGui::Text(TR("layer_tooltip_visible"), isVisible ? TR("layer_yes") : TR("layer_no"));
            ImGui::Text(TR("layer_tooltip_active"), isActive ? TR("layer_yes") : TR("layer_no"));
            ImGui::Text("%s", TR("layer_tooltip_left_click"));
            ImGui::Text("%s", TR("layer_tooltip_right_click"));
            ImGui::EndTooltip();
        }
    }
...
std::string UI::GetAssetTypeName(const AssetType type) {
    switch (type) {
        case AssetType::INDIVIDUAL_TEXTURE: return TR("asset_type.texture");
        case AssetType::ANIMATED_SPRITESHEET: return TR("asset_type.animated_sprite");
        case AssetType::STATIC_SPRITESHEET: return TR("asset_type.static_sprite");
        case AssetType::TILESET: return TR("asset_type.tileset");
        case AssetType::PLAYER: return TR("asset_type.player");
        case AssetType::ENEMY: return TR("asset_type.enemy");
        default: return TR("asset_type.other");
    }
}
...
void UI::RenderTileSelectionWindow(Engine& engine, Asset* tileset) {
    ...
    if (ImGui::Begin(TR("tile_selection.window_title"), &showTileSelectionWindow, ImGuiWindowFlags_NoScrollbar)) {
        ImGui::Text(TR("tile_selection.select_tile_from"), tileset->name.c_str());
        ImGui::Text(TR("tile_selection.tileset_info"),
                   tileset->texture.width, tileset->texture.height,
                   tileset->subSprites.size());
        ImGui::Separator();

        // Tile size configuration
        ImGui::Text("%s", TR("tile_selection.tile_size"));
        ImGui::SameLine();
        ImGui::SetNextItemWidth(80);
        ImGui::InputInt(TR("tile_selection.width"), &tilesetTileWidth, 1, 10);
        ImGui::SameLine();
        ImGui::SetNextItemWidth(80);
        ImGui::InputInt(TR("tile_selection.height"), &tilesetTileHeight, 1, 10);
        ...
        if (selectedTileIndex >= 0 && selectedTileIndex < tileset->subSprites.size()) {
            ...
            ImGui::Text(TR("tile_selection.selected_tile"),
            ...
            ImGui::Text("%s", TR("tile_selection.no_tile_selected"));
        }
...
void UI::RenderTilesetGrid(Engine& engine, Asset* tileset) {
    if (!tileset->loaded || tileset->subSprites.empty()) {
        ImGui::Text("%s", TR("tile_selection.tileset_not_loaded"));
        return;
    }

    ImGui::Text("%s", TR("tile_selection.click_tile_to_select"));
    ...
    for (int y = 0; y < rows; y++) {
        for (int x = 0; x < columns; x++) {
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::BeginTooltip();
                ImGui::Text(TR("tile_selection.tile_index"), tileIndex);
                ImGui::Text(TR("tile_selection.position"), x, y);
                ImGui::Text(TR("tile_selection.coords"), tileRect.x, tileRect.y);
                ImGui::EndTooltip();
            }
...
void UI::RenderTileAnimationEditorWindow(Engine& engine) {
    static int selectedFrameEditorDefIndex = -1;
    static int selectedFrameIndex = -1;
    static char selectedFrameAssetBuffer[128] = "";
    static int selectedFrameTileIndex = 0;
    static double selectedFrameDuration = 0.15;

    ImGui::SetNextWindowSize(ImVec2(760, 520), ImGuiCond_FirstUseEver);
    if (!ImGui::Begin(TR("tile_anim.window_title"), &showTileAnimationEditorWindow)) {
        ImGui::End();
        return;
    }

    auto loadSelectedDefToEditorState = [&](const Engine::AnimatedTileDefinition& def) {
        strncpy_s(tileAnimationIdBuffer, def.id.c_str(), sizeof(tileAnimationIdBuffer) - 1);
        tileAnimationIdBuffer[sizeof(tileAnimationIdBuffer) - 1] = '\0';
        tileAnimationLoop = def.loop;

        if (!def.baseTiles.empty()) {
            strncpy_s(tileAnimationBaseAssetBuffer, def.baseTiles[0].assetID.c_str(), sizeof(tileAnimationBaseAssetBuffer) - 1);
            tileAnimationBaseAssetBuffer[sizeof(tileAnimationBaseAssetBuffer) - 1] = '\0';
    ...
    if (selectedFrameEditorDefIndex != selectedTileAnimationDefIndex) {
        selectedFrameEditorDefIndex = selectedTileAnimationDefIndex;
        selectedFrameIndex = -1;
        selectedFrameAssetBuffer[0] = '\0';
        selectedFrameTileIndex = 0;
        selectedFrameDuration = 0.15;
    }

    if (ImGui::Button(TR("tile_anim.new_definition"), ImVec2(140, 28))) {
    ...
    if (selectedTileAnimationDefIndex >= 0) {
        if (ImGui::Button(TR("tile_anim.delete_selected"), ImVec2(140, 28))) {
            std::vector<Engine::AnimatedTileDefinition> definitions(definitionsRef.begin(), definitionsRef.end());
    ...
    ImGui::Text("%s", TR("tile_anim.definitions"));
    ...
    ImGui::NextColumn();
    ImGui::Text("%s", TR("tile_anim.editor"));
    ...
    if (selectedTileAnimationDefIndex < 0 || selectedTileAnimationDefIndex >= static_cast<int>(definitionsRef.size())) {
        ImGui::TextDisabled("%s", TR("tile_anim.select_definition_to_edit"));
        ImGui::EndChild();
        ImGui::Columns(1);
        ImGui::End();
        return;
    ...
    ImGui::InputText(TR("tile_anim.id"), tileAnimationIdBuffer, sizeof(tileAnimationIdBuffer));
    ImGui::Checkbox(TR("tile_anim.loop"), &tileAnimationLoop);

    ImGui::Separator();
    ImGui::Text("%s", TR("tile_anim.base_tile"));
    ImGui::InputText(TR("tile_anim.base_asset_id"), tileAnimationBaseAssetBuffer, sizeof(tileAnimationBaseAssetBuffer));
    ImGui::InputInt(TR("tile_anim.base_tile_index"), &tileAnimationBaseIndex);
    tileAnimationBaseIndex = std::max(0, tileAnimationBaseIndex);

    if (ImGui::Button(TR("tile_anim.use_currently_selected_tile"), ImVec2(220, 0))) {
        if (selectedTileset && selectedTileIndex >= 0) {
            strncpy_s(tileAnimationBaseAssetBuffer, selectedTileset->id.c_str(), sizeof(tileAnimationBaseAssetBuffer) - 1);
            tileAnimationBaseAssetBuffer[sizeof(tileAnimationBaseAssetBuffer) - 1] = '\0';
            tileAnimationBaseIndex = selectedTileIndex;
        } else {
    ...
    if (ImGui::Button(TR("tile_anim.apply_definition_header"), ImVec2(220, 0))) {
    ...
    ImGui::Separator();
    ImGui::Text("%s", TR("tile_anim.frames"));
    int removeFrameIndex = -1;
    for (int i = 0; i < static_cast<int>(selectedDef.frames.size()); ++i) {
        const auto& frame = selectedDef.frames[i];
        ImGui::PushID(i);
        std::string frameLabel = std::to_string(i) + "##FrameSelect";
        if (ImGui::Selectable(frameLabel.c_str(), selectedFrameIndex == i, ImGuiSelectableFlags_SpanAllColumns)) {
            selectedFrameIndex = i;
            strncpy_s(selectedFrameAssetBuffer, frame.assetID.c_str(), sizeof(selectedFrameAssetBuffer) - 1);
            selectedFrameAssetBuffer[sizeof(selectedFrameAssetBuffer) - 1] = '\0';
            selectedFrameTileIndex = std::max(0, frame.tileIndex);
            selectedFrameDuration = std::max(0.01, frame.duration);
        }
        ImGui::SameLine();
        ImGui::Text(TR("tile_anim.frame_line"), i, frame.assetID.c_str(), frame.tileIndex, frame.duration);
        if (ImGui::Button(TR("tile_anim.remove"))) {
            removeFrameIndex = i;
        }
        ImGui::PopID();
    }
    ...
    if (removeFrameIndex >= 0) {
        ...
        if (removeFrameIndex < static_cast<int>(def.frames.size())) {
            ...
            engine.SetAnimatedTileDefinitions(definitions);
            if (selectedFrameIndex == removeFrameIndex) {
                selectedFrameIndex = -1;
            } else if (selectedFrameIndex > removeFrameIndex) {
                --selectedFrameIndex;
            }
    ...
    if (selectedFrameIndex >= static_cast<int>(selectedDef.frames.size())) {
        selectedFrameIndex = -1;
    }

    ImGui::Separator();
    ImGui::Text("%s", TR("tile_anim.edit_selected_frame"));
    if (selectedFrameIndex < 0 || selectedFrameIndex >= static_cast<int>(selectedDef.frames.size())) {
        ImGui::TextDisabled("%s", TR("tile_anim.select_frame_to_edit"));
    } else {
        const std::string selectedFrameAssetLabel = std::string(TR("tile_anim.frame_asset_id")) + "##SelectedFrameEdit";
        const std::string selectedFrameTileIndexLabel = std::string(TR("tile_anim.frame_tile_index")) + "##SelectedFrameEdit";
        const std::string selectedFrameDurationLabel = std::string(TR("tile_anim.frame_duration_seconds")) + "##SelectedFrameEdit";
        ImGui::InputText(selectedFrameAssetLabel.c_str(), selectedFrameAssetBuffer, sizeof(selectedFrameAssetBuffer));
        ImGui::InputInt(selectedFrameTileIndexLabel.c_str(), &selectedFrameTileIndex);
        ImGui::InputDouble(selectedFrameDurationLabel.c_str(), &selectedFrameDuration, 0.000001, 0.001, "%.10f");
        selectedFrameTileIndex = std::max(0, selectedFrameTileIndex);
        selectedFrameDuration = std::max(0.01, selectedFrameDuration);

        const std::string selectedFrameUseTileLabel = std::string(TR("tile_anim.use_selected_tile_for_frame")) + "##SelectedFrameEdit";
        if (ImGui::Button(selectedFrameUseTileLabel.c_str(), ImVec2(220, 0))) {
            if (selectedTileset && selectedTileIndex >= 0) {
                strncpy_s(selectedFrameAssetBuffer, selectedTileset->id.c_str(), sizeof(selectedFrameAssetBuffer) - 1);
                selectedFrameAssetBuffer[sizeof(selectedFrameAssetBuffer) - 1] = '\0';
                selectedFrameTileIndex = selectedTileIndex;
            } else {
                SetDebugMessage("[TILE ANIM] Select a tile in the Tile Selection window first");
            }
        }

        const std::string selectedFrameApplyLabel = std::string(TR("tile_anim.apply_frame_changes")) + "##SelectedFrameEdit";
        if (ImGui::Button(selectedFrameApplyLabel.c_str(), ImVec2(180, 0))) {
            if (strlen(selectedFrameAssetBuffer) == 0) {
                SetDebugMessage("[TILE ANIM] Frame Asset ID is required");
            } else {
                std::vector<Engine::AnimatedTileDefinition> definitions(definitionsRef.begin(), definitionsRef.end());
                auto& def = definitions[selectedTileAnimationDefIndex];
                if (selectedFrameIndex >= 0 && selectedFrameIndex < static_cast<int>(def.frames.size())) {
                    auto& frame = def.frames[selectedFrameIndex];
                    frame.assetID = selectedFrameAssetBuffer;
                    frame.tileIndex = selectedFrameTileIndex;
                    frame.duration = selectedFrameDuration;
                    engine.SetAnimatedTileDefinitions(definitions);
                    SetDebugMessage("[TILE ANIM] Updated selected frame");
                }
            }
        }
    }
    ...
    ImGui::Separator();
    ImGui::Text("%s", TR("tile_anim.add_frame"));
    ImGui::InputText(TR("tile_anim.frame_asset_id"), tileAnimationFrameAssetBuffer, sizeof(tileAnimationFrameAssetBuffer));
    ImGui::InputInt(TR("tile_anim.frame_tile_index"), &tileAnimationFrameIndex);
    ImGui::InputDouble(TR("tile_anim.frame_duration_seconds"), &tileAnimationFrameDuration, 0.000001, 0.001, "%.10f");
    tileAnimationFrameIndex = std::max(0, tileAnimationFrameIndex);
    tileAnimationFrameDuration = std::max(0.01, tileAnimationFrameDuration);

    if (ImGui::Button(TR("tile_anim.use_selected_tile_for_frame"), ImVec2(220, 0))) {
        if (selectedTileset && selectedTileIndex >= 0) {
            strncpy_s(tileAnimationFrameAssetBuffer, selectedTileset->id.c_str(), sizeof(tileAnimationFrameAssetBuffer) - 1);
    ...
    if (ImGui::Button(TR("tile_anim.add_frame_button"), ImVec2(120, 0))) {
...
void UI::RenderPlayModeWindow(Engine& engine) {
    ...
    if (ImGui::Begin(TR("play_mode"), &engine.playModeWindowOpen, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize)) {
        const ImVec2 contentSize = ImGui::GetContentRegionAvail();
        const std::string fps = std::string(TR("fps_label")) + std::to_string(io.Framerate);
...
void UI::RenderModeControls(Engine& engine) {
    ImGui::Separator();
    ImGui::Text("%s", TR("engine_mode"));

    std::string editModeLabel = std::string(TR("edit_mode")) + "##EditModeRadio";
    if (ImGui::RadioButton(editModeLabel.c_str(), engine.currentMode == Engine::Mode::EDIT)) {
        engine.StopPlayMode();
        engine.ResetTool();
    }
    ImGui::SameLine();
    std::string playModeLabel = std::string(TR("play_mode")) + "##PlayModeRadio";
    if (ImGui::RadioButton(playModeLabel.c_str(), engine.currentMode == Engine::Mode::PLAY)) {
    ...
    if (engine.currentMode == Engine::Mode::EDIT) {
        ImGui::Separator();
        ImGui::Text("%s", TR("removal_tools"));

        ImGui::SameLine();
        if (ImGui::Button(TR("remove_player"), ImVec2(100, 0))) {
        ...
        ImGui::SameLine();
        if (ImGui::Button(TR("remove_enemy"), ImVec2(100, 0))) {
        ...
        ImGui::Text("%s", TR("current_tool"));
        ImGui::SameLine();
        switch (engine.currentTool) {
            case Engine::ToolState::NONE: ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.8f, 1.0f), TR("tool_none")); break;
            case Engine::ToolState::PLACING_PLAYER: ImGui::TextColored(ImVec4(0.2f, 0.8f, 0.2f, 1.0f), TR("tool_placing_player")); break;
            case Engine::ToolState::REMOVING_PLAYER: ImGui::TextColored(ImVec4(0.8f, 0.2f, 0.2f, 1.0f), TR("tool_removing_player")); break;
            case Engine::ToolState::PLACING_ENEMY: ImGui::TextColored(ImVec4(0.2f, 0.8f, 0.2f, 1.0f), TR("tool_placing_enemy")); break;
            case Engine::ToolState::REMOVING_ENEMY: ImGui::TextColored(ImVec4(0.8f, 0.2f, 0.2f, 1.0f), TR("tool_removing_enemy")); break;
            case Engine::ToolState::PLACING_TILE: ImGui::TextColored(ImVec4(0.2f, 0.8f, 0.2f, 1.0f), TR("tool_placing_tile")); break;
            case Engine::ToolState::REMOVING_TILE: ImGui::TextColored(ImVec4(0.8f, 0.2f, 0.2f, 1.0f), TR("tool_removing_tile")); break;
            case Engine::ToolState::PLACING_ASSET: ImGui::TextColored(ImVec4(0.2f, 0.8f, 0.2f, 1.0f), TR("tool_placing_asset")); break;
            case Engine::ToolState::REMOVING_ASSET: ImGui::TextColored(ImVec4(0.8f, 0.2f, 0.2f, 1.0f), TR("tool_removing_asset")); break;
        }

        // Show enemy count
        ImGui::Text(TR("enemies_count"), engine.GetEnemyCount());

        if (ImGui::Button(TR("clear_tool"), ImVec2(120, 0))) {
            engine.ResetTool();
        }
        ImGui::Separator();
        ImGui::Text("%s", TR("project"));
        static char filenameBuf[256] = "project.json";
        std::string pathInputLabel = std::string(TR("path_input")) + "##ProjectPath";
        ImGui::InputText(pathInputLabel.c_str(), filenameBuf, sizeof(filenameBuf));
...
void UI::RenderCameraResolutionControls(Engine& engine) {
    ImGui::Separator();
    ImGui::Text("%s", TR("player_camera_settings"));

    // Use public method to check if player exists
    if (!engine.HasPlayer()) {
        ImGui::TextColored(ImVec4(1.0f, 0.5f, 0.5f, 1.0f), "%s", TR("place_player_for_camera_preview"));
    } else {
        ImGui::Text("%s", TR("camera_preview_visible"));
    }

    // Resolution dropdown using public methods
    ImGui::Text("%s", TR("resolution"));
...
for (const auto& res : resolutions) {
    ...
    if (currentIndex >= 0 && currentIndex < resolutions.size()) {
        ...
        ImGui::Text(TR("current_resolution"),
        ...
        const char* aspectName = TR("custom");
        ...
        ImGui::Text(TR("aspect_ratio"), aspectName);
    }
}
...
void UI::RenderNewSceneTab(Engine& engine) {
    ...
    for (int i = 0; i < sceneCount; i++) {
        ...
        if (ImGui::BeginPopupContextItem("SceneContext")) {
            ...
            ImGui::Text("%s", TR("scene_options"));
            ImGui::Separator();

            // Rename
            ImGui::Text("%s", TR("scene_name_label"));
            ImGui::SameLine();
            ImGui::SetNextItemWidth(150.0f);
            ImGui::InputText("##SceneRename", renameBuf, sizeof(renameBuf));

            if (ImGui::Button(TR("apply_rename"))) {
            ...
            if (ImGui::Checkbox(TR("starting_scene"), &isStarting)) {
                if (isStarting) {
